...
export const AnalyzerClient = () => {
    ...
    try {
      ...
      supabase.functions
        ...
        .then(({ data, error: speedErr }) => {
          if (speedErr || !data) {
            setSpeed({ status: "unavailable", reason: "Speed data unavailable for this site right now. Try again in a moment." });
            return;
          }
          if (data.unavailable) {
            setSpeed({ status: "unavailable", reason: data.reason || "Speed data unavailable" });
          } else {
            setSpeed({ status: "ready", score: data.score, max: data.max, checks: data.checks, vitals: data.vitals });
          }
        })
        .catch(() => {
          setSpeed({ status: "unavailable", reason: "Speed data unavailable for this site right now. Try again in a moment." });
      ...
      const { data, error: invokeErr } = await supabase.functions.invoke("analyze-website", { body: { url } });
      if (invokeErr) {
  ...
  const mergedOverall = (() => {
    ...
    if (speed.status === "ready") {
      return base.categories.seo.score + base.categories.aiVisibility.score + base.categories.security.score + speed.score;
    }
  ...
  const mergedIssues = (() => {
    if (!base) return [];
    if (speed.status === "ready") return [...base.issues, ...speed.checks.filter((c) => !c.passed)];
    return base.issues;
  })();

  const mergedPassed = (() => {
    if (!base) return [];
    if (speed.status === "ready") return [...base.passed, ...speed.checks.filter((c) => c.passed)];
    return base.passed;
  })();
      ...
      <div className="container relative">
        <section className="max-w-3xl mx-auto text-center mb-12">
          <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 mb-6 border border-accent/30 bg-accent/5">
            ...
            <span className="text-[10px] uppercase tracking-[0.2em] text-accent font-medium">Free Website Audit</span>
          </div>
          ...
          <p className="text-muted-foreground text-base md:text-lg leading-relaxed mb-8 font-light">
            Enter your URL below for an instant breakdown of your SEO, performance, mobile experience, and security, with specific fixes for every issue we find.
          </p>
        ...
        {base && !loading && (
          <div className="animate-fade-up" style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            <section className="max-w-2xl mx-auto rounded-3xl border border-border bg-card/40 px-6 sm:px-10 text-center flex flex-col items-center" style={{ paddingTop: "48px", paddingBottom: "48px" }}>
              ...
              <CategoryCard
                ...
                unavailableCta={
                  speed.status === "unavailable"
                    ? { label: "We test every site we build for speed before launch. Ask us about it", href: "https://byf-vercel-depo.vercel.app/" }
                    : undefined
                }
              />
              <CategoryCard
            ...
            <section className="relative rounded-3xl p-[2px] bg-gradient-to-br from-accent via-primary to-accent bg-[length:200%_200%] animate-gradient-shift" style={{ marginBottom: "48px" }}>
              <div className="rounded-3xl bg-background py-20 px-6 sm:px-10 md:py-24 md:px-14 text-center">
                ...
                <p className="text-muted-foreground max-w-2xl mx-auto mb-3 leading-relaxed">
                  We've helped dozens of small businesses go from a broken digital presence to one that actually wins customers. Your score shows exactly where you're losing, and we know exactly how to fix it.
                </p>
                <p className="mb-10" style={{ fontSize: "1.1rem" }}>
                  <span className="text-muted-foreground">Your site scored </span>
                  <span className="font-display font-medium" style={{ color: "#c9a84c" }}>{mergedOverall}/100</span>
