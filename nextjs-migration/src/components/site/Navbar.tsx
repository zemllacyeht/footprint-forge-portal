    ...
    >
      ...
      {open && (
        <div className="md:hidden container mt-4 glass rounded-xl p-6 space-y-4 animate-fade-up">
          {links.map((l) => (
            <Link key={l.href} href={l.href} onClick={() => setOpen(false)} className="block text-foreground/80 hover:text-primary">
              {l.label}
            </Link>
          ))}
          <div className="pt-4 border-t border-border flex flex-col gap-2">
            <Button variant="hero" size="sm" asChild><Link href="/contact">Start a Project</Link></Button>
          </div>
        </div>
      )}
    </header>
